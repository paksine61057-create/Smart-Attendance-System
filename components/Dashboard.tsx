...
const Dashboard: React.FC = () => {
  ...
  // Handle Export PDF with even more aggressive Auto-Scaling to ensure single page
  const handleExportPDF = (type: 'daily' | 'monthly') => {
    const doc = new jsPDF('p', 'mm', 'a4');
    const data = type === 'daily' ? officialData : monthlyLatenessData;
    const rowCount = data.length;
    
    // Compressive scale for high row counts
    let fontSize = 7.5;
    let cellPadding = 1.5;
    
    if (rowCount > 25) {
      fontSize = Math.max(5.5, 7.5 - Math.floor((rowCount - 25) / 6));
      cellPadding = Math.max(0.6, 1.5 - ((rowCount - 25) / 20));
    }
    ...
    // Highly compact header (Y starts at 10)
    doc.setFontSize(12);
    ...
    (doc as any).autoTable({
        ...
        didDrawPage: (data: any) => {
            ...
            doc.setFontSize(6);
            doc.text(`Generated by SchoolCheckIn AI - ${new Date().toLocaleString('th-TH')}`, 105, pageHeight - 4, { align: 'center' });
        }
    });

    // Ultra-tight signature block
    const finalY = (doc as any).lastAutoTable.finalY || 20;
    const sigY = Math.min(finalY + 5, 280); 
    
    doc.setFontSize(fontSize);
    doc.text('(ลงชื่อ)...........................................................', 55, sigY, { align: 'center' });
    doc.text('กลุ่มบริหารงานบุคคล', 55, sigY + 3.5, { align: 'center' });

    doc.text('(ลงชื่อ)...........................................................', 155, sigY, { align: 'center' });
    doc.text('ผู้อำนวยการโรงเรียนประจักษ์ศิลปาคม', 155, sigY + 3.5, { align: 'center' });
  ...
  const webFontSize = useMemo(() => {
    const count = staffList.length;
    if (count > 25) return Math.max(6.5, 8.5 - Math.floor((count - 25) / 6)) + 'px';
    return '9px';
  }, [staffList.length]);
      ...
      <div className="bg-white rounded-[3.5rem] shadow-2xl border-4 border-rose-50 overflow-hidden min-h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'official' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[4mm] md:p-[8mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-1">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-9 h-9 md:w-11 md:h-11 object-contain mb-1" />
                   <h1 className="text-xs md:text-sm font-black text-stone-900 leading-tight max-w-lg uppercase">รายงานการมาปฏิบัติงานของครูและบุคลากรทางการศึกษา</h1>
                   <h1 className="text-xs md:text-sm font-black text-stone-900 leading-tight uppercase">โรงเรียนประจักษ์ศิลปาคม</h1>
                   <h2 className="text-[10px] font-bold text-stone-700 mt-0.5">ประจำวันที่ {new Date(selectedDate).toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' })}</h2>
                </div>
                <div className="flex-1 mt-2">
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-10">ลำดับ</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-28 md:w-30">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-16 md:w-20">เวลามา</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-16 md:w-20">เวลากลับ</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-36 md:w-40">หมายเหตุ</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
                         {officialData.map(d => (
                            <tr key={d.no} className="hover:bg-stone-50/30">
                               <td className="border border-stone-300 p-0.5 text-center font-mono">{d.no}</td>
                               <td className="border border-stone-300 p-0.5 text-left whitespace-nowrap font-bold text-stone-800 pl-2">{d.name}</td>
                               <td className="border border-stone-300 p-0.5 text-center text-stone-500 font-medium">{d.role}</td>
                               <td className="border border-stone-300 p-0.5 text-center font-normal text-stone-900">{d.arrival}</td>
                               <td className="border border-stone-300 p-0.5 text-center font-normal text-stone-900">{d.departure}</td>
                               <td className="border border-stone-300 p-0.5 text-center text-stone-500 italic font-medium">{d.remark}</td>
                            </tr>
                         ))}
                      </tbody>
                   </table>
                </div>
                {/* Compact Signatures Web View */}
                <div className="mt-1 grid grid-cols-2 gap-2 text-center border-t border-transparent py-1">
                   <div>
                      <p className="text-[8px] text-stone-800 mb-1">(ลงชื่อ)...........................................................</p>
                      <p className="text-[8px] font-bold text-stone-400 uppercase">กลุ่มบริหารงานบุคคล</p>
                   </div>
                   <div>
                      <p className="text-[8px] text-stone-800 mb-1">(ลงชื่อ)...........................................................</p>
                      <p className="text-[8px] font-bold text-stone-400 uppercase">ผู้อำนวยการโรงเรียนประจักษ์ศิลปาคม</p>
                   </div>
                </div>
             </div>
          </div>
        ...
        {activeTab === 'monthly' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[4mm] md:p-[8mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-1">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-9 h-9 md:w-11 md:h-11 object-contain mb-1" />
                   <h1 className="text-xs md:text-sm font-black text-stone-900 leading-tight max-w-xl uppercase">รายงานการมาสายของครูและบุคลากรทางการศึกษา</h1>
                   <h1 className="text-xs md:text-sm font-black text-stone-900 leading-tight uppercase">โรงเรียนประจักษ์ศิลปาคม</h1>
                   <h2 className="text-[10px] font-bold text-stone-700 mt-0.5">ประจำเดือน {new Date(selectedDate).toLocaleDateString('th-TH', { month: 'long', year: 'numeric' })}</h2>
                </div>
                <div className="flex-1 mt-2">
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-10">ลำดับ</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-30">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-20">ขาด/ไม่ลงชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-20">จำนวนมาสาย</th>
                            <th className="border border-stone-300 p-1 text-[8px] font-black text-center w-40">วันที่ที่มาสาย</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
                         {monthlyLatenessData.map(d => (
                            <tr key={d.no} className="hover:bg-stone-50/30">
                               ...
                               <td className="border border-stone-300 p-0.5 text-left whitespace-nowrap font-bold text-stone-800 pl-2">{d.name}</td>
                               <td className="border border-stone-300 p-0.5 text-center text-stone-500 font-medium">{d.role}</td>
                               <td className={`border border-stone-300 p-0.5 text-center font-black ${d.absentCount > 0 ? 'text-orange-600' : 'text-stone-300'}`}>{d.absentCount || '-'}</td>
                               <td className={`border border-stone-300 p-0.5 text-center font-black ${d.lateCount > 0 ? 'text-rose-600' : 'text-stone-300'}`}>{d.lateCount || '-'}</td>
                               <td className="border border-stone-300 p-0.5 text-center text-stone-500 italic font-medium break-words">{d.lateDates}</td>
                            </tr>
                         ))}
                      </tbody>
                   </table>
