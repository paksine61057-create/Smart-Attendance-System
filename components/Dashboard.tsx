...
const Dashboard: React.FC = () => {
  ...
  const handleExportPDF = (type: 'daily' | 'monthly') => {
    ...
    // Aggressive Auto-scale
    let fontSize = 8.5;
    let cellPadding = 2;
    
    if (rowCount > 20) {
      fontSize = Math.max(6, 8.5 - Math.floor((rowCount - 20) / 4));
      cellPadding = Math.max(1, 2 - ((rowCount - 20) / 12));
    }
    ...
    (doc as any).autoTable({
        ...
        margin: { top: 24, bottom: 40 }, // Reserve space for signatures at bottom
        didDrawPage: (data: any) => {
            const pageHeight = doc.internal.pageSize.height;
            doc.setFontSize(7);
            doc.text(`Generated by SchoolCheckIn AI - ${new Date().toLocaleString('th-TH')}`, 105, pageHeight - 6, { align: 'center' });
        }
    });

    // Add Signatures immediately after table
    const finalY = (doc as any).lastAutoTable.finalY || 24;
    const sigY = Math.min(finalY + 12, 270); // Ensure it doesn't spill off page if possible
    
    doc.setFontSize(fontSize);
    doc.text('(ลงชื่อ)...........................................................', 55, sigY, { align: 'center' });
    doc.text('กลุ่มบริหารงานบุคคล', 55, sigY + 5, { align: 'center' });

    doc.text('(ลงชื่อ)...........................................................', 155, sigY, { align: 'center' });
    doc.text('ผู้อำนวยการโรงเรียนประจักษ์ศิลปาคม', 155, sigY + 5, { align: 'center' });
      ...
      <div className="bg-white rounded-[3.5rem] shadow-2xl border-4 border-rose-50 overflow-hidden min-h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'official' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[8mm] md:p-[12mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-4">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-12 h-12 md:w-14 md:h-14 object-contain mb-3" />
                   <h1 className="text-md md:text-lg font-black text-stone-900 leading-tight max-w-lg">รายงานการมาปฏิบัติงานของครูและบุคลากรทางการศึกษา</h1>
                   <h1 className="text-md md:text-lg font-black text-stone-900 leading-tight">โรงเรียนประจักษ์ศิลปาคม</h1>
                   <h2 className="text-sm font-bold text-stone-700 mt-1">ประจำวันที่ {new Date(selectedDate).toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' })}</h2>
                </div>
                <div className="flex-1">
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-12">ลำดับที่</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-28 md:w-32">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-20 md:w-24">เวลามา</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-20 md:w-24">เวลากลับ</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-40 md:w-44">หมายเหตุ</th>
                         </tr>
                      </thead>
                ...
                <div className="mt-4 grid grid-cols-2 gap-8 text-center">
                   <div>
                      <p className="text-[10px] text-stone-800 mb-6">(ลงชื่อ)...........................................................</p>
        ...
        {activeTab === 'monthly' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[8mm] md:p-[12mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-4">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-12 h-12 md:w-14 md:h-14 object-contain mb-3" />
                   <h1 className="text-md md:text-lg font-black text-stone-900 leading-tight max-w-xl">รายงานการมาสายของครูและบุคลากรทางการศึกษา</h1>
                   <h1 className="text-md md:text-lg font-black text-stone-900 leading-tight">โรงเรียนประจักษ์ศิลปาคม</h1>
                   <h2 className="text-sm font-bold text-stone-700 mt-1">ประจำเดือน {new Date(selectedDate).toLocaleDateString('th-TH', { month: 'long', year: 'numeric' })}</h2>
                </div>
                <div className="flex-1">
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-12">ลำดับที่</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-32">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-24">จำนวนครั้งที่ไม่ลงเวลา</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-24">จำนวนครั้งที่มาสาย</th>
                            <th className="border border-stone-300 p-1 text-[10px] font-black text-center w-48">วันที่ที่มาสาย</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
