...
const Dashboard: React.FC = () => {
  ...
  // Handle Export PDF with Auto-Scaling and Adjusted Header position
  const handleExportPDF = (type: 'daily' | 'monthly') => {
    ...
    // Decreased base size to 9 for better fit
    let fontSize = 9;
    let cellPadding = 2.5;
    
    if (rowCount > 20) {
      fontSize = Math.max(6, 9 - Math.floor((rowCount - 20) / 4));
      cellPadding = Math.max(1, 2.5 - ((rowCount - 20) / 12));
    }
    ...
    // Move headers further up (School Name Y: 12, Title Y: 19)
    doc.setFontSize(14);
    ...
    (doc as any).autoTable({
        startY: 25, // Move table start up from 35 to 25
        head: headers,
        body: body,
        theme: 'grid',
        styles: {
        ...
        didDrawPage: (data: any) => {
            ...
            doc.setFontSize(7);
            doc.text(`Generated by SchoolCheckIn AI - ${new Date().toLocaleString('th-TH')}`, 105, pageHeight - 8, { align: 'center' });
        }
    });
  ...
  const webFontSize = useMemo(() => {
    ...
    if (count > 25) return Math.max(7, 10 - Math.floor((count - 25) / 5)) + 'px';
    return '10px';
  }, [staffList.length]);
      ...
      <div className="bg-white rounded-[3.5rem] shadow-2xl border-4 border-rose-50 overflow-hidden min-h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'official' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[10mm] md:p-[15mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-6">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-14 h-14 md:w-16 md:h-16 object-contain mb-4" />
                   ...
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-12">ลำดับที่</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-28 md:w-32">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-20 md:w-24">เวลามา</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-20 md:w-24">เวลากลับ</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-40 md:w-44">หมายเหตุ</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
                         {officialData.map(d => (
                            <tr key={d.no} className="hover:bg-stone-50/50">
                               ...
                               <td className="border border-stone-300 p-1.5 text-left whitespace-nowrap font-bold text-stone-800 pl-3">{d.name}</td>
                   ...
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-12">ลำดับที่</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-32">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-24">จำนวนครั้งที่ไม่ลงเวลา</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-24">จำนวนครั้งที่มาสาย</th>
                            <th className="border border-stone-300 p-1.5 text-[10px] font-black text-center w-48">วันที่ที่มาสาย</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
