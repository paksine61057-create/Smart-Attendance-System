...
const Dashboard: React.FC = () => {
  ...
  // Handle Export PDF with Advanced Auto-Scaling
  const handleExportPDF = (type: 'daily' | 'monthly') => {
    const doc = new jsPDF('p', 'mm', 'a4');
    const data = type === 'daily' ? officialData : monthlyLatenessData;
    const rowCount = data.length;
    
    // Aggressive Auto-scale for single page fit
    let fontSize = 8;
    let cellPadding = 1.8;
    
    if (rowCount > 25) {
      fontSize = Math.max(6, 8 - Math.floor((rowCount - 25) / 5));
      cellPadding = Math.max(0.8, 1.8 - ((rowCount - 25) / 15));
    }
    ...
    (doc as any).autoTable({
        ...
        margin: { top: 24, bottom: 30 }, // Reduced bottom margin
        didDrawPage: (data: any) => {
            const pageHeight = doc.internal.pageSize.height;
            doc.setFontSize(6);
            doc.text(`Generated by SchoolCheckIn AI - ${new Date().toLocaleString('th-TH')}`, 105, pageHeight - 5, { align: 'center' });
        }
    });

    // Tightened Signature Logic
    const finalY = (doc as any).lastAutoTable.finalY || 24;
    const sigY = Math.min(finalY + 6, 275); // Move signatures up: finalY + 6 instead of + 12
    
    doc.setFontSize(fontSize);
    doc.text('(ลงชื่อ)...........................................................', 55, sigY, { align: 'center' });
    doc.text('กลุ่มบริหารงานบุคคล', 55, sigY + 4, { align: 'center' });

    doc.text('(ลงชื่อ)...........................................................', 155, sigY, { align: 'center' });
    doc.text('ผู้อำนวยการโรงเรียนประจักษ์ศิลปาคม', 155, sigY + 4, { align: 'center' });
  ...
  // Compute dynamic font size for web view (A4 simulation)
  const webFontSize = useMemo(() => {
    const count = staffList.length;
    if (count > 25) return Math.max(7, 9 - Math.floor((count - 25) / 5)) + 'px';
    return '9.2px';
  }, [staffList.length]);
      ...
      <div className="bg-white rounded-[3.5rem] shadow-2xl border-4 border-rose-50 overflow-hidden min-h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'official' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[6mm] md:p-[10mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-2">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-10 h-10 md:w-12 md:h-12 object-contain mb-2" />
                   <h1 className="text-sm md:text-md font-black text-stone-900 leading-tight max-w-lg">รายงานการมาปฏิบัติงานของครูและบุคลากรทางการศึกษา</h1>
                   <h1 className="text-sm md:text-md font-black text-stone-900 leading-tight">โรงเรียนประจักษ์ศิลปาคม</h1>
                   <h2 className="text-xs font-bold text-stone-700 mt-1">ประจำวันที่ {new Date(selectedDate).toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' })}</h2>
                </div>
                <div className="flex-1">
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-10">ลำดับที่</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-28 md:w-32">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-20 md:w-24">เวลามา</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-20 md:w-24">เวลากลับ</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-40 md:w-44">หมายเหตุ</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
                         {officialData.map(d => (
                            <tr key={d.no} className="hover:bg-stone-50/50">
                               <td className="border border-stone-300 p-1 text-center font-mono">{d.no}</td>
                               <td className="border border-stone-300 p-1 text-left whitespace-nowrap font-bold text-stone-800 pl-3">{d.name}</td>
                               <td className="border border-stone-300 p-1 text-center text-stone-500 font-medium">{d.role}</td>
                               <td className="border border-stone-300 p-1 text-center font-normal text-stone-900">{d.arrival}</td>
                               <td className="border border-stone-300 p-1 text-center font-normal text-stone-900">{d.departure}</td>
                               <td className="border border-stone-300 p-1 text-center text-stone-500 italic font-medium">{d.remark}</td>
                            </tr>
                         ))}
                      </tbody>
                   </table>
                </div>
                {/* Tightened Signatures Web View */}
                <div className="mt-2 grid grid-cols-2 gap-4 text-center border-t border-transparent">
                   <div className="py-2">
                      <p className="text-[9px] text-stone-800 mb-2">(ลงชื่อ)...........................................................</p>
                      <p className="text-[9px] font-bold text-stone-400 uppercase">กลุ่มบริหารงานบุคคล</p>
                   </div>
                   <div className="py-2">
                      <p className="text-[9px] text-stone-800 mb-2">(ลงชื่อ)...........................................................</p>
                      <p className="text-[9px] font-bold text-stone-400 uppercase">ผู้อำนวยการโรงเรียนประจักษ์ศิลปาคม</p>
                   </div>
                </div>
             </div>
          </div>
        ...
        {activeTab === 'monthly' && (
          <div className="p-0 md:p-12 bg-stone-100 min-h-screen relative">
             ...
             <div className="max-w-[210mm] mx-auto bg-white shadow-2xl p-[6mm] md:p-[10mm] min-h-[297mm] flex flex-col border border-stone-200">
                <div className="flex flex-col items-center text-center mb-2">
                   <img src={SCHOOL_LOGO_URL} alt="School Logo" className="w-10 h-10 md:w-12 md:h-12 object-contain mb-2" />
                   <h1 className="text-sm md:text-md font-black text-stone-900 leading-tight max-w-xl">รายงานการมาสายของครูและบุคลากรทางการศึกษา</h1>
                   <h1 className="text-sm md:text-md font-black text-stone-900 leading-tight">โรงเรียนประจักษ์ศิลปาคม</h1>
                   <h2 className="text-xs font-bold text-stone-700 mt-1">ประจำเดือน {new Date(selectedDate).toLocaleDateString('th-TH', { month: 'long', year: 'numeric' })}</h2>
                </div>
                <div className="flex-1">
                   <table className="w-full border-collapse border border-stone-300">
                      <thead>
                         <tr className="bg-stone-50">
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-10">ลำดับที่</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center">รายชื่อ</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-32">ตำแหน่ง</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-24">จำนวนครั้งที่ไม่ลงเวลา</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-24">จำนวนครั้งที่มาสาย</th>
                            <th className="border border-stone-300 p-1 text-[9px] font-black text-center w-48">วันที่ที่มาสาย</th>
                         </tr>
                      </thead>
                      <tbody style={{ fontSize: webFontSize }}>
                         {monthlyLatenessData.map(d => (
                            <tr key={d.no} className="hover:bg-stone-50/50">
                               ...
                               <td className="border border-stone-300 p-1 text-left whitespace-nowrap font-bold text-stone-800 pl-3">{d.name}</td>
                               <td className="border border-stone-300 p-1 text-center text-stone-500 font-medium">{d.role}</td>
                               <td className={`border border-stone-300 p-1 text-center font-black ${d.absentCount > 0 ? 'text-orange-600' : 'text-stone-300'}`}>{d.absentCount || '-'}</td>
                               <td className={`border border-stone-300 p-1 text-center font-black ${d.lateCount > 0 ? 'text-rose-600' : 'text-stone-300'}`}>{d.lateCount || '-'}</td>
                               <td className="border border-stone-300 p-1 text-center text-stone-500 italic font-medium break-words max-w-[200px]">{d.lateDates}</td>
                            </tr>
                         ))}
                      </tbody>
                   </table>
